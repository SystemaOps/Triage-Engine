...
// ── Category theme map (adjusted for dark mode) ──
...
export default function ReportManagementView({ userRole }: { userRole: Role }) {
  ...
  if (loading) {
    return (
      <div className="flex h-[80vh] items-center justify-center font-mono text-xs text-slate-400 animate-pulse tracking-widest bg-zinc-900">
  ...
  if (reports.length === 0) {
    return (
      <div className="space-y-6 p-6 max-w-[1600px] mx-auto animate-fade-in bg-zinc-900 text-slate-200">
        <div className="border-b border-slate-700 pb-5">
          <h2 className="text-2xl font-bold tracking-tight text-slate-100">Diagnostic &amp; Audit Ledger</h2>
          <p className="text-sm text-slate-400">Radiology logs, lab results, OCR transcriptions, and symptom analysis queue.</p>
        </div>
        <div className="flex flex-col items-center justify-center border border-dashed border-slate-700 rounded-2xl bg-zinc-800/50 p-16 text-center">
          <svg className="w-12 h-12 text-slate-600 mb-4" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor">
  ...
  return (
    <div className="h-[calc(100vh-7rem)] p-6 max-w-[1600px] mx-auto animate-fade-in flex flex-col bg-zinc-900 text-slate-200">
      {/* ── Header ── */}
      <div className="flex items-start justify-between flex-shrink-0 pb-4 border-b border-slate-700">
        <div>
          <h2 className="text-2xl font-bold tracking-tight text-slate-100">Diagnostic &amp; Audit Ledger</h2>
          <p className="text-sm text-slate-400">Radiology logs, lab results, OCR transcriptions, and symptom analysis queue.</p>
        </div>
        <div className="flex items-center gap-2 font-mono text-[11px] bg-zinc-800 border border-slate-700 px-3 py-1.5 rounded-xl text-slate-400 shadow-sm">
      ...
      <div className="grid grid-cols-3 gap-4 py-4 flex-shrink-0">
        {[
          { title: 'Pending Review', value: pendingCount, color: 'text-amber-400', bg: 'bg-amber-900' },
          { title: 'Verified Today', value: verifiedTodayCount, color: 'text-emerald-400', bg: 'bg-emerald-900' },
          { title: 'Flagged', value: flaggedCount, color: 'text-rose-400', bg: 'bg-rose-900' },
        ].map((card, i) => (
          <div key={i} className="bg-zinc-800 border border-slate-700 rounded-2xl p-4 shadow-sm">
      ...
      <div className="flex-1 min-h-0 grid grid-cols-1 lg:grid-cols-5 gap-6">
        ...
        <div className="lg:col-span-3 flex flex-col min-h-0">
              ...
              <button
                ...
                className={`px-3 py-1.5 text-[11px] font-bold font-mono uppercase tracking-wider rounded-lg border transition-all whitespace-nowrap ${
                  filterCategory === cat
                    ...
                    : 'bg-zinc-800 text-slate-400 border-slate-700 hover:border-slate-600'
                }`}
              >
          ...
          <div className="flex-1 overflow-y-auto space-y-2 pr-1">
            {filteredReports.length === 0 ? (
              <div className="flex items-center justify-center h-full border border-dashed border-slate-700 rounded-xl bg-zinc-800/50">
                <span className="text-xs font-mono text-slate-500">No reports match the selected filter.</span>
              </div>
            ) : (
              filteredReports.map((report) => {
                ...
                return (
                  <div
                    ...
                    className={`bg-zinc-800 border rounded-xl p-4 cursor-pointer transition-all duration-150 ${
                      isSelected
                  ...
                  >
                    <div className="flex items-start justify-between">
                      <div className="flex items-center gap-2 min-w-0">
                        <h4 className="font-bold text-sm text-slate-100 truncate">{patientDisplayName(userRole, report.patientName)}</h4>
                    ...
                    <div className="flex items-center gap-4 mt-2 text-[11px] text-slate-500 font-mono">
                      ...
                      <span className={`font-bold ${report.confidence >= 0.8 ? 'text-emerald-400' : report.confidence >= 0.5 ? 'text-amber-400' : 'text-rose-400'}`}>
        ...
        <div className="lg:col-span-2 flex flex-col gap-6">
            {/* Data Visualization */}
            <div className="bg-zinc-800 border border-slate-700 rounded-2xl shadow-sm p-4 h-60 flex flex-col">
                <h3 className="text-md font-bold text-slate-100 mb-2">Report Category Distribution</h3>
                <ResponsiveContainer width="100%" height="100%" minWidth={0}>
                    ...
                    >
                        ...
                        <YAxis stroke="#94a3b8" />
                        <Tooltip cursor={{ fill: 'rgba(255,255,255,0.1)' }} contentStyle={{ backgroundColor: '#1e293b', border: '1px solid #475569', color: '#e2e8f0' }} />
            ...
            {/* Report Details */}
            <div className="flex-1 bg-zinc-800 border border-slate-700 rounded-2xl shadow-sm overflow-y-auto">
              {selectedReport ? (
                <div className="p-5 space-y-5">
                  ...
                  <div>
                    <div className="flex items-start justify-between">
                      <div>
                        <h3 className="text-lg font-bold text-slate-100">{patientDisplayName(userRole, selectedReport.patientName)}</h3>
                  ...
                  <div>
                    <div className="flex justify-between items-center mb-1">
                      <span className="text-[11px] font-bold uppercase text-slate-400">AI Confidence</span>
                      <span className="text-xs font-mono font-bold text-slate-200">{Math.round(selectedReport.confidence * 100)}%</span>
                    </div>
                  ...
                  {selectedReport.content.rawText && (
                    <div>
                      <h4 className="text-[11px] font-bold uppercase tracking-wider text-slate-400 mb-2">Raw Extraction</h4>
                      <div className="bg-zinc-900 border border-slate-700 rounded-xl p-4 text-xs font-mono text-slate-300 whitespace-pre-wrap leading-relaxed max-h-[200px] overflow-y-auto">
                        {selectedReport.content.rawText}
                      </div>
                    </div>
                  )}
                  ...
                  {selectedReport.content.structuredData && Object.keys(selectedReport.content.structuredData).length > 0 && (
                    <div>
                      <h4 className="text-[11px] font-bold uppercase tracking-wider text-slate-400 mb-2">Extracted Metrics</h4>
                      <div className="space-y-1.5">
                        {Object.entries(selectedReport.content.structuredData).map(([key, value]) => (
                          <div key={key} className="flex justify-between items-center bg-zinc-900/60 rounded-lg px-3 py-1.5 border border-slate-800">
                            <span className="text-xs font-mono text-slate-400 capitalize">{key.replace(/_/g, ' ')}</span>
                            <span className="text-xs font-bold font-mono text-slate-200">{String(value)}</span>
                          </div>
                        ))}
                      </div>
                    </div>
                  ...
                  {selectedReport.content.aiAnalysis && (
                    <div>
                      <h4 className="text-[11px] font-bold uppercase tracking-wider text-slate-400 mb-2">AI Analysis</h4>
                      <div className="bg-indigo-900/60 border border-indigo-700 rounded-xl p-4 text-xs text-slate-300 leading-relaxed">
                        {selectedReport.content.aiAnalysis}
                      </div>
                    </div>
                  )}
                  ...
                  <div className="flex items-center justify-between">
                    {selectedReport.status === 'verified' && selectedReport.verifiedBy ? (
                      <div className="text-[11px] text-slate-500 font-mono">
                        Verified by <span className="font-bold text-slate-300">{selectedReport.verifiedBy.substring(0, 8)}</span>
