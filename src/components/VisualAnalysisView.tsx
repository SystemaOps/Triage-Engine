...
import { Role, VisualAnalyzeResponse, TriageResult } from '../types';
import { Upload, FileText, Activity, CheckCircle, AlertCircle, Loader2, X, Search, Eye, BrainCircuit, Save, AlertTriangle, ArrowRight } from 'lucide-react';
...
const TARGETS = [
  { id: 'skin', label: 'Skin', desc: 'Lesions / Rash / Dermatological' },
  { id: 'eyes', label: 'Eyes', desc: 'Ocular symptoms / Conjunctivitis' },
  { id: 'nails', label: 'Nails', desc: 'Nail abnormalities / Infections' },
  { id: 'tongue', label: 'Tongue', desc: 'Oral symptoms / Tongue conditions' },
];
...
export default function VisualAnalysisView({ userRole }: VisualAnalysisViewProps) {
  const [file, setFile] = useState<File | null>(null);
  const [previewUrl, setPreviewUrl] = useState<string | null>(null);
  const [phase, setPhase] = useState<Phase>('idle');
  const [result, setResult] = useState<VisualAnalyzeResponse | null>(null);
  const [errorMessage, setErrorMessage] = useState<string | null>(null);
  const [target, setTarget] = useState('skin');
  ...
  const handleProcess = async () => {
    ...
    setErrorMessage(null);
    try {
      const r = await api.visual.analyze(file, file.name, target);
      setResult(r);
      setPhase('complete');
    } catch (err) {
      setPhase('error');
  ...
  const handleRunTriage = async () => {
    ...
    setTriageError(null);
    try {
      const symptomsText = chiefComplaint.trim() || findings.substring(0, 500) || 'Visual symptom analysis requested';
      const triage = await api.llm.triage({
        symptoms: symptomsText,
        patient_case: `Visual Symptom Analysis (${TARGETS.find(t => t.id === target)?.label}):\n${fullText}`.substring(0, 2000),
  ...
  const handleSaveWithTriage = async () => {
    ...
    try {
      ...
      const rawData = Object.entries(flat)
        .map(([key, value]) => `${key}: ${value}`)
        .join('\n');
      await api.visual.saveReportWithTriage({
        patientId: patientId.trim(),
        patientName: patientName.trim() || 'Unknown Patient',
        target,
        rawData,
      ...
      console.error('Failed to save Visual report with triage:', err);
    }
  };
    ...
    return (
      <div className="mx-auto max-w-4xl animate-fade-in">
        <div className="border-b border-slate-200 pb-5 mb-8">
          <h2 className="text-2xl font-bold tracking-tight text-slate-900">Visual Symptom Analysis</h2>
          <p className="text-sm text-slate-500 mt-1">Upload a clinical photo to detect findings and indicators.</p>
        </div>

        <div className="mb-6">
          <label className="block text-[11px] font-semibold text-slate-500 uppercase tracking-wider mb-2">Anatomy Target</label>
          <div className="grid grid-cols-4 gap-3">
            {TARGETS.map(t => (
              <button key={t.id} onClick={() => setTarget(t.id)}
                className={`px-4 py-3 rounded-xl text-left border transition-all ${
                  target === t.id ? 'bg-emerald-50 border-emerald-300 text-emerald-700' : 'bg-white border-slate-200 text-slate-600 hover:border-slate-300'
                }`}>
        ...
        <div onDrop={handleDrop} onDragOver={e => e.preventDefault()}
          ...
          className="border-2 border-dashed border-slate-300 rounded-2xl bg-slate-50 hover:bg-white hover:border-emerald-400 transition-all cursor-pointer p-16 text-center group">
          <div className="flex flex-col items-center gap-4">
            ...
            <p className="text-base font-semibold text-slate-700">Drop symptom photo here<br /><span className="text-sm font-normal text-slate-500">or click to browse</span></p>
  ...
  return (
    <div className="mx-auto max-w-5xl animate-fade-in">
      <div className="border-b border-slate-200 pb-5 mb-8 flex items-center justify-between">
        ...
        <button onClick={resetAll} className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium text-slate-500 bg-white border border-slate-200 rounded-lg hover:bg-slate-50"><X size={14} /> New Photo</button>
      </div>
        ...
        <div>
          <div className="bg-white border border-slate-200 rounded-2xl overflow-hidden shadow-sm">
            ...
            <div className="bg-slate-50 flex items-center justify-center p-4">
              {previewUrl && <img src={previewUrl} alt="Symptom photo" className="max-w-full max-h-[400px] object-contain rounded-lg shadow-sm" />}
            </div>
          </div>
          <div className="mt-3 flex items-center gap-2 px-3 py-2 bg-emerald-50 border border-emerald-200 rounded-lg text-xs text-emerald-700">
            <Eye size={14} /> Target: {TARGETS.find(t => t.id === target)?.label}
          </div>
          <button onClick={handleProcess} disabled={phase === 'processing'}
            ...
            {phase === 'processing' ? <><Loader2 size={18} className="animate-spin" /> Analyzing...</> : <><BrainCircuit size={18} /> Analyze Photo</>}
          </button>
        </div>
        ...
        <div className="space-y-4">
          {phase === 'processing' && (
            <div className="bg-emerald-50 border border-emerald-200 rounded-2xl p-8 text-center">
              ...
              <h3 className="text-base font-semibold text-emerald-700">Analyzing Photo</h3>
              <p className="text-sm text-emerald-500 mt-1">Analyzing via unified triage API pipeline...</p>
            </div>
          )}
          {phase === 'error' && (
            <div className="bg-rose-50 border border-rose-200 rounded-2xl p-6 flex items-start gap-3">
              ...
              <div><h3 className="text-sm font-semibold text-rose-700">Analysis Failed</h3><p className="text-xs text-rose-600 mt-1">{errorMessage}</p></div>
            </div>
          )}
            ...
            <>
              <div className="bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden">
                <div className="px-5 py-3 border-b border-slate-100 flex items-center gap-2">
                  ...
                  <span className="text-sm font-bold text-slate-800">Analysis Results</span>
              ...
              {/* ════════════════════════════════════════════ */}
              <div className="bg-gradient-to-br from-emerald-50 to-teal-50 border border-emerald-200 rounded-2xl overflow-hidden shadow-sm">
                <div className="px-5 py-3 border-b border-emerald-100 flex items-center gap-2">
                  ...
                  <span className="text-sm font-bold text-emerald-800">LLM Triage Analysis</span>
                  ...
                  <div className="p-5 space-y-4">
                    <div>
                      ...
                      <textarea
                        ...
                        placeholder="e.g., rash on arm for 3 days, itchy, red"
                        rows={2}
                        className="w-full text-sm border border-slate-200 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 bg-white text-slate-700 placeholder:text-slate-400 resize-none"
                      />
                      <p className="text-[10px] text-slate-400 mt-1.5">The analysis findings will be sent for triage. Add a chief complaint for context.</p>
                    </div>
                    <button onClick={handleRunTriage}
                      className="w-full py-2.5 text-sm font-semibold text-white bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-700 hover:to-teal-700 rounded-xl transition-all flex items-center justify-center gap-2 shadow-lg shadow-emerald-500/20">
                ...
                {triagePhase === 'running' && (
                  <div className="p-8 text-center">
                    ...
                    <h3 className="text-sm font-semibold text-emerald-700">Analyzing with LLM</h3>
                    <p className="text-xs text-emerald-500 mt-1">Running triage on visual findings...</p>
                  </div>
                )}
                  ...
                  <div className="p-5">
                    <div className="flex items-start gap-3">
                      ...
                      <div>
                        ...
                        <button onClick={() => setTriagePhase('idle')} className="mt-2 text-xs font-medium text-emerald-600 hover:text-emerald-700">Try again</button>
                      </div>
                    </div>
                  </div>
                )}
                ...
                {triagePhase === 'complete' && triageResult && (
                  <div className="p-5 space-y-4">
                    ...
                    <button onClick={() => setTriagePhase('idle')}
                      className="w-full py-2 text-xs font-medium text-emerald-600 bg-white border border-emerald-200 hover:bg-emerald-50 rounded-lg transition-all">
              ...
              <div className="bg-white border border-slate-200 rounded-2xl shadow-sm p-5 space-y-4">
                <h3 className="text-sm font-bold text-slate-800 flex items-center gap-2"><Save size={16} className="text-emerald-500" /> Save to Firestore</h3>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    ...
                    <input type="text" value={patientId} onChange={(e) => setPatientId(e.target.value)}
                      ...
                      className="w-full text-sm border border-slate-200 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 bg-white text-slate-700 placeholder:text-slate-400" />
                  </div>
                  <div>
                ...
                <button onClick={handleSaveReport} disabled={saveStatus === 'saving' || saveStatus === 'saved' || !patientId.trim()}
                  ...
                  {saveStatus === 'saving' ? <><Loader2 size={16} className="animate-spin" /> Saving...</> : saveStatus === 'saved' ? <><CheckCircle size={16} /> Report Saved</> : <><Save size={16} /> Save as Symptom Report</>}
                </button>
                ...
                {triagePhase === 'complete' && triageResult && (
                  <>
                    ...
                    <button onClick={handleSaveWithTriage} disabled={saveWithTriageStatus === 'saving' || saveWithTriageStatus === 'saved' || !patientId.trim()}
                      className={`w-full py-2.5 text-sm font-semibold rounded-xl transition-all flex items-center justify-center gap-2 ${
                        saveWithTriageStatus === 'saved' ? 'bg-emerald-50 text-emerald-600 border border-emerald-200' : 'bg-emerald-600 text-white hover:bg-emerald-700 disabled:opacity-50 shadow-lg shadow-emerald-500/20'
                      }`}>
          ...
          {phase === 'idle' && !file && (
            <div className="flex flex-col items-center justify-center h-full border border-dashed border-slate-200 rounded-2xl bg-slate-50 p-8 text-center">
              ...
              <p className="text-sm font-medium text-slate-500">Select an anatomy target and upload a symptom photo</p>
            </div>
          )}
        </div>
      </div>
